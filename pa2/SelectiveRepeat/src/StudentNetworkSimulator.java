...
public class StudentNetworkSimulator extends NetworkSimulator
{
    ...
    private int rcvd = 0;	// Number of packet received
    private int numlost = 0;
    private int pktLayer5 = 0;
    ...
    private int seqBase;     // The start of the window
    private int seqLast;     // The end of the window
    ...
    private int totalDeliveredPacket;    // Number of packets successfully delivered to B(All packet included)
    int B_Application = 0;
    int B_Transition = 0;

    private LinkedList<Double> Ttime;   // Linked List of transmitted time for sent packets
    private LinkedList<Double> cStartTime;
    private LinkedList<Double> cEndTime;
    ...
    protected double averageCom()
    {
        double Commean = 0;
        int i;
        for(i = 0; i < seqBase; i++){
            Commean += (cEndTime.get(i) - cStartTime.get(i));
        }
        
        return Commean = Commean / i;
    }
    ...
    protected void aOutput(Message message)
    {
        ...
        if(Psent.indexOf(p) <= seqLast){
            ...
            cStartTime.add(Psent.indexOf(p), getTime()); 
            totalSentPacket++; 
    ...
    protected void aInput(Packet packet)
    {
        ...
        while(seqBase < Psent.size()){
            ...
            if(Psent.get(seqBase).getSeqnum() == packet.getAcknum()){ //ACK is for the last sent packet
                break;
            }
            
            Ttime.add(seqBase, time - Ttime.get(seqBase)); //Each ACK is recorded in the linked list for next check
            cStartTime.add(seqBase, time - cStartTime.get(seqBase));
            seqBase++;
            seqLast++;
        ...
        cStartTime.add(seqBase, time - Ttime.get(seqBase));
        seqBase++;
        seqLast++;
    ...
    protected void aTimerInterrupt()
    {
        ...
        for(int i = seqBase; ((i <= seqLast) &&  (i < Psent.size())); i++){           
            ...
            if(i == nextPacket){ //First time sending a specific packet
                Ttime.add(i, time);
                nextPacket++;
            }
    ...
    protected void aInit()
    {
        ...
        cStartTime = new LinkedList<Double>();
    }
    ...
    {
    	totaltime = getTime();
    	numlost = totalSentPacket - totalDeliveredPacket;
    	// TO PRINT THE STATISTICS, FILL IN THE DETAILS BY PUTTING VARIBALE NAMES. DO NOT CHANGE THE FORMAT OF PRINTED OUTPUT
    	System.out.println("\n\n===============STATISTICS=======================");
    	System.out.println("Number of original packets transmitted by A:" + nextPacket);
    	System.out.println("Number of retransmissions by A:" + (totalSentPacket - nextPacket));
    	System.out.println("Number of data packets delivered to layer 5 at B:" + pktLayer5);
    	System.out.println("Number of ACK packets sent by B:" + numACK);
    	System.out.println("Number of corrupted packets:" + numCorrupt);
    	System.out.println("Ratio of lost packets:" + ((double)  (numlost)/totalSentPacket));
    	System.out.println("Ratio of corrupted packets:" + ((double) numCorrupt/(totalSentPacket-numlost)));
